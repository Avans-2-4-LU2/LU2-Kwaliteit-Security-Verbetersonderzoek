...
**Status:** Current state of branch `feature/audit-report` as of 2026-06-19. Every claim below is
backed by a specific artifact (file, commit, test result) — per this report's guiding principle:
**"A report without evidence is just an opinion."**

## Rubric Coverage Map

This report is structured against the **"LU2 Verbeteronderzoek Security Beroepsproduct"** rubric.
Each graded criterion maps to a specific section:

| Rubric Criterion (points) | Addressed in |
|---|---|
| Security audit (legislation & standards) — /20 | §2 Scope & Context, §4 Findings, §8 Traceability Matrix |
| Secure pipelines — /15 | §6 Secure Pipelines |
| Update advice (SBOM, CVE and CVSS) — /15 | §5 SBOM & Supply Chain Security |
| Security code review & vulnerabilities — /15 | §4 Findings, §9 AI Tooling Accountability |
| Penetration tests — /15 | §4, Finding B-001 |
| Mitigation & validation of improvements — /20 | §4 "Evidence after fix" rows, §6 |

(The separate "Maintainability" rubric is a different deliverable and is not addressed here.)
...
shouldn't see them; this is planned for this sprint.

**Roadmap:**

| Priority | Action |
|---|---|
| Now | — (no critical issues currently open) |
| This sprint | Restrict free-text appointment notes from unauthorized staff |
| Later | Add tests proving the remaining permission checks correctly deny unauthorized users; tidy up one minor CI configuration detail |
...
| **Status** | **Open.** No work has started; planned for this sprint. |
...
|---|---|---|
| Branch protection / required checks | ✅ | CodeQL (×2), Dependency Review, SBOM & SCA, SonarCloud, Java CI with Maven all required on PRs |
| SAST on every PR | ✅ | CodeQL Advanced workflows |
| SCA / dependency alerts | ✅ | Dependabot + manual triage (§5) |
...
- **Appendix L** — Snyk report: **[TODO — this project uses CodeQL + Dependabot, not Snyk. Confirm with
  the rubric owner whether the SCA evidence above (Appendix B, C) is an acceptable substitute.]**
...
2. Resolve the Snyk-report question (Appendix L).
3. Decide whether B-005 gets work this sprint, or is formally accepted as residual risk with sign-off.
4. Get sign-off / independent review on this report from the rest of the project group (NEN-7510 5.35
   requires independent review — this should not be one person's unilateral claim of compliance).
