        ...
        if (StringUtils.hasText(text)) {
            try {
                ...
                if (ts == null) {
                    ...
                    log.warn("[SECURITY] BINDING_ERROR - User: [" + username + "] submitted invalid TimeSlot identifier: [" + sanitizedInput + "]");
                    
                    // 4. Throw a generic exception without exposing internal error messages
                    throw new IllegalArgumentException("TimeSlot not found for provided input.");
                }
            }
        } else {
            setValue(null);
